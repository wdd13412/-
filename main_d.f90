...
! 调用CFD计算，传入变形网格点，输出结果到cellPrimitives
							  ...
							  ncells = 18513
								n = ncells*5
								ALLOCATE(b_i(n), x_tan(n), dwdx(n, 5), rhs_P(ncells, 5))
								tol = 3.0d-50!tol = 1.0d-8
								maxiter = 50
								m_restart = 50
								! ===== 新增：建預條件（一次即可）=====
							    ...
							    !b_i=-∂R/∂MACH
							    !这里CALL TANGENT_RHS_MACH(data_4d137, cellprimitivesout, machnum, n, b_i)
							    !CALL RHS_MACH_VERIFY(data_4d137, cellprimitivesout, machnum, n, b_i)
							    !STOP 'RHS_MACH_VERIFY done.'
							    !rhs_P(1:ncells, 5) = b_i(1:ncells)
   								!x_tan = 0.0_8
   								!无雅可比矩阵GMRES方法(正向自动微分求解矩阵向量积)求解dw/dx
   								!CALL SOLVE_TANGENT_JFGMRES(data_4d137, cellprimitivesout, n, b_i, x_tan, tol, maxiter, m_restart, info)
    							dwdx(1:n, 5) = 0.0_8
								rhs_P(1:ncells, 5) = 0.0_8!这里实际上是对mach的导数
								...
								print *, "ABD"
								CALL OUTPUTVTK_D(meshpath, cellprimitivesout, dwdx, 'solution_001', point_update, rhs_P=rhs_P)
								print *, "ABD"
								DEALLOCATE(b_i, x_tan, dwdx, rhs_P)
! 释放point_update内存        
