...
! 日後可用解析 JST-L1 係數寫入 pc_blk_inv，介面不變。
  ...
  ! 預設關：與 matrix-free 切線一致（原 .TRUE. 僅在 PC 組裝暫開會造成 M≠A）
  LOGICAL, SAVE :: pc_use_hicken_a1 = .FALSE.
  REAL(kind=8), SAVE :: pc_hicken_sigma = 5.0d0
  ! 着色 AD 构建阶段的稳定优先策略（仅影响预处理矩阵组装，不影响主残差定义）
  LOGICAL, SAVE :: pc_colored_stable_mode = .TRUE.
  LOGICAL, SAVE :: pc_colored_force_hicken = .TRUE.
  REAL(kind=8), SAVE :: pc_colored_hicken_sigma = 5.0d0
  ! 着色AD构建后的块过滤（仅作用于预条件矩阵构建阶段）
  REAL(kind=8), SAVE :: pc_colored_drop_rel = 1.0d-12
  REAL(kind=8), SAVE :: pc_colored_offdiag_cap = 20.0d0
  ...
  LOGICAL, SAVE :: pc_use_amp_guard = .TRUE.
  REAL(kind=8), SAVE :: pc_amp_guard_ratio = 5.0d1
  LOGICAL, SAVE :: pc_colored_use_diag_only = .TRUE.
  ...
  ! 当前预条件矩阵是否来自着色 AD（与 Krylov 中矩阵-free 算子可能不一致，勿叠 AM1 / 两水平）
  LOGICAL, SAVE :: pc_matrix_from_colored_ad = .FALSE.
  LOGICAL, SAVE :: pc_colored_light_apply = .TRUE.
  REAL(kind=8), SAVE :: pc_colored_dd_frac = 0.45d0
  ! 着色 AD 块对角加强（PC_POSTPROCESS）；相对残差卡住时可试 3e-3～3e-2（愈大愈像「强对角」ILU）
  REAL(kind=8), SAVE :: pc_colored_diag_stab = 5.0d-3
	...
	&       ' ||r||/||b||: ', rnrm_aug_before/bnrm, ' -> ', rnrm_aug_after/bnrm
		...
		DO j = 1, m_restart
		  ...
		  ELSE
		    z(1:n) = v(1:n, j)
		  END IF
	      IF (pc_active .AND. k == 1 .AND. j == 1) THEN
...
&   dwdx, ncells, csvfilename)
  ...
  SUBROUTINE BUILD_CELL_COLORING_L2(ncells, nlist, row_cnt, color, ncolor)
    ...
    INTEGER(kind=8) :: c, ii, jj, n1, n2, k
  ...
  END SUBROUTINE BUILD_PC_MATRIX_COLORED_AD
	  ...
	  PRINT *, '[PC] BUILD_PC_JST_L1: 固定采用着色 AD + L1 窄模板构建', &
		& ' call=', pc_build_calls, ' consistency_mode=', pc_dafoam_consistency_mode, &
		& ' amp_guard_ratio=', pc_amp_guard_ratio
