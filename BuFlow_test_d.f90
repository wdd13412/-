...
! 日後可用解析 JST-L1 係數寫入 pc_blk_inv，介面不變。
  ...
  LOGICAL, SAVE :: pc_ready = .FALSE.
    ! .TRUE.：COMPUTE_STEADY_RESIDUAL_D 使用 UNSTRUCTURED_JSTEPSS_D（耗散切线近邻/截断远端链，供预处理等）
...
!   (2) 用 UNSTRUCTURED_JSTEPS 只更新 epss 原值，epssd、eps22d 置零：冻结 ε 对 W 的切线（类论文启动阶段）。
  SUBROUTINE UNSTRUCTURED_JSTEPSS_D(boundaryconditions, fluid)
    IMPLICIT NONE
    TYPE(BOUNDARYCONDITION), INTENT(IN) :: boundaryconditions(:)
    TYPE(FLUIDD), INTENT(IN) :: fluid
    REAL(kind=8) :: d(3), grad_v(3), dot_grad_v
...
&         diffusionflux(v)*unitfa
    ...
    END DO
    CALL INTEGRATEFLUXES_UNSTRUCTURED3D_D()
  END SUBROUTINE UNSTRUCTURED_JSTEPSS_D
...
!		allocate(fluxResidualss(nCells, 5), source=0.0_8)
    IF (use_jstepss_for_steady_residual_d) THEN
      CALL UNSTRUCTURED_JSTEPSS_D(boundaryconditions, fluid)
    ELSE
...
&   dwdx, ncells, csvfilename)
  ...
  END SUBROUTINE OUTPUT_WALL_CP_CSV
	  ...
	  INTEGER(kind=8) :: mvar, rvar, rcell
	  INTEGER(kind=8) :: min_deg, max_deg, sum_deg, diag_missing, asym_edges
	  INTEGER(kind=8) :: test_cells(3), tix, outside_nnz, max_abs_cell, max_abs_var
	  INTEGER(kind=8) :: total_calls, calls_with_leak, total_outside_nnz
	  REAL(kind=8), ALLOCATABLE :: cellprimitivesd(:, :)
	  REAL(kind=8) :: data_4d137d(1,4)
	  REAL(kind=8) :: inside_l1, outside_l1, total_l1, max_abs_val
	  REAL(kind=8) :: max_leak_abs, max_leak_ratio, leak_ratio
	  LOGICAL :: ok
	  LOGICAL, ALLOCATABLE :: rowmark(:)
	  REAL(kind=8) :: ainfn, dampv
	  REAL(kind=8), PARAMETER :: pc_diag_tau = 1.0d-3
	  ...
	  ! [DBG-1] 結構檢查：L1 圖是否自洽（含對角、度數、對稱性）
	  min_deg = HUGE(1_8)
	  max_deg = 0_8
	  sum_deg = 0_8
	  diag_missing = 0_8
	  asym_edges = 0_8
	  DO c = 1_8, ncells
		min_deg = MIN(min_deg, row_cnt(c))
		max_deg = MAX(max_deg, row_cnt(c))
		sum_deg = sum_deg + row_cnt(c)
		IF (.NOT. ANY(nlist(c,1:row_cnt(c)) == c)) diag_missing = diag_missing + 1_8
	  END DO
	  DO c = 1_8, ncells
		DO i = 1_8, row_cnt(c)
		  rcell = nlist(c,i)
		  ...
		  IF (rcell == c) CYCLE
		  IF (.NOT. ANY(nlist(rcell,1:row_cnt(rcell)) == c)) asym_edges = asym_edges + 1_8
		END DO
	  END DO
	  PRINT *, '[DBG-PC1] L1-struct: ncells=', ncells, ' nnzRows(sum)=', sum_deg, ' minRow=', min_deg, ' maxRow=', max_deg, ' diagMissing=', diag_missing, ' asymHalfEdges=', asym_edges
	  ...
	  END DO
	  	  ...
	  	  ! ------------------------------------------------------------
	  ! 2) Per-column forward AD: same path as TANGENT_MATVEC (full JSTFLUX_D,
	  !    not JSTEPSS). One seed (cell c, var mvar) per call avoids wrong
	  !    column sums from coloring vs wide JST stencil. Values are still
	  !    injected only at level-1 (self + face neighbours).
	  ! ------------------------------------------------------------
	  ALLOCATE(cellprimitivesd(ncells,5))
	  ALLOCATE(rowmark(ncells))
	  data_4d137d = 0.0_8
	  PRINT *, '[PC-AD] per-column AD, full JSTFLUX_D (same as matvec), ncells=', ncells
	  max_leak_abs = 0.0_8
	  max_leak_ratio = 0.0_8
	  total_calls = 0_8
	  calls_with_leak = 0_8
	  total_outside_nnz = 0_8
	  test_cells(1) = 1_8
	  test_cells(2) = MAX(1_8, (ncells+1_8)/2_8)
	  test_cells(3) = ncells
	  ...
	  DO c = 1_8, ncells
	    DO mvar = 1_8, 5_8
	      ...
	      CALL COMPUTE_STEADY_RESIDUAL_D(data_4d137, data_4d137d, cellprimitives, cellprimitivesd)
	      total_calls = total_calls + 1_8

	      rowmark = .FALSE.
	      rowmark(nlist(c,1:row_cnt(c))) = .TRUE.
	      inside_l1 = 0.0_8
	      outside_l1 = 0.0_8
	      outside_nnz = 0_8
	      max_abs_val = 0.0_8
	      ...
	      DO rcell = 1_8, ncells
		DO rvar = 1_8, 5_8
		  ainfn = ABS(fluxresiduals_slnd(rcell, rvar))
		  IF (rowmark(rcell)) THEN
		    inside_l1 = inside_l1 + ainfn
		  ELSE
		    outside_l1 = outside_l1 + ainfn
		    IF (ainfn > 0.0_8) outside_nnz = outside_nnz + 1_8
		  END IF
		  IF (ainfn > max_abs_val) THEN
		    max_abs_val = ainfn
		    max_abs_cell = rcell
		    max_abs_var = rvar
		  END IF
		END DO
	      END DO
	      total_l1 = inside_l1 + outside_l1
	      IF (outside_nnz > 0_8) calls_with_leak = calls_with_leak + 1_8
	      total_outside_nnz = total_outside_nnz + outside_nnz
	      IF (outside_l1 > max_leak_abs) max_leak_abs = outside_l1
	      IF (total_l1 > 0.0_8) THEN
		leak_ratio = outside_l1 / total_l1
		IF (leak_ratio > max_leak_ratio) max_leak_ratio = leak_ratio
	      END IF

	      DO tix = 1_8, 3_8
		IF (c == test_cells(tix) .AND. mvar == 1_8) THEN
		  PRINT *, '[DBG-PC2] seed(c,m)=', c, mvar, ' insideL1=', inside_l1, ' outsideL1=', outside_l1, ' outsideNNZ=', outside_nnz, ' maxAbs@=', max_abs_cell, max_abs_var, ' maxAbs=', max_abs_val
		END IF
	      END DO
	  ...
	  ! [DBG-3] 全域總結：若 leak 顯著，代表「結構正交/局部支撐」假設不成立
	  PRINT *, '[DBG-PC3] AD-support summary: calls=', total_calls, ' callsWithLeak=', calls_with_leak, ' totalOutsideNNZ=', total_outside_nnz, ' maxOutsideL1=', max_leak_abs, ' maxLeakRatio=', max_leak_ratio
	  ...
	  DO c = 1_8, ncells
		...
		IF (p > 0_8) THEN
		  ...
		  DO i = 1_8, 5_8
		    ...
		    END DO
		  END DO
		  dampv = pc_diag_tau * MAX(1.0_8, ainfn)
		  DO i = 1_8, 5_8
		    pc_blk(p,i,i) = pc_blk(p,i,i) + dampv
		  END DO
		END IF
	  END DO
