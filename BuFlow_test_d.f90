...
!$AD NONDIFF  ! 明确该变量为非微分
    CHARACTER(len=256) :: vtkfilename
    INTEGER(kind=8) :: ncells, npoints, i
    REAL(kind=8), INTENT(IN), OPTIONAL :: rhs_P(:, :)
    INTRINSIC SIZE
    INTRINSIC PRESENT
    INTRINSIC TRIM
...
&                 13_int64, -1_int64, 12_int64/), rhs_P=rhs_P)
    ELSE
      CALL WRITEVTKFILE_D(vtkfilename, pointlocations, cells, &
&                 cellprimitives, cellprimitivesd, npoints, ncells, (/ &
&                 1_int64, 3_int64, 5_int64, 10_int64, 14_int64, &
...
&   cellprimitivesd, npts, ncells, vtktypes, rhs_P)
    IMPLICIT NONE
    CHARACTER(len=*), INTENT(IN) :: filename
! 3×nPts（x,y,z）
    REAL(kind=8), INTENT(IN) :: points(:, :)
...
! [cell1:P,T,Ux,Uy,Uz, cell2:P,T,Ux,Uy,Uz, ...]
      ...
      DO j = 1, nparams
        ...
        END DO
      END DO
            ! RHS - dR/dx 的 P 分量，第5维命名为 RHS_P_mach
      IF (PRESENT(rhs_P)) THEN
        DO j = 1, SIZE(rhs_P, 2)
          IF (j .LE. 4) THEN
          ...
          WRITE(fileunit, '(A)') 'LOOKUP_TABLE default'
          DO i = 1, ncells
            WRITE(fileunit, '(F16.8)') rhs_P(i, j)
          END DO
        END DO
      END IF
      CLOSE(fileunit)
...
!		real(kind=8), intent(inout) :: fluxResiduals(:,:)
    ...
    IF (dsum_design .LE. 1.0d-20) THEN
