...
! 日後可用解析 JST-L1 係數寫入 pc_blk_inv，介面不變。
  ...
  LOGICAL, SAVE :: pc_full_ad_pattern = .FALSE.
    ! .TRUE.：组装完 pc_blk 后将所有非对角 5×5 块置 0，仅保留 (cell,cell) 对角块（用于诊断非对角过小/奇异导致的 ILU 问题）
  LOGICAL, SAVE :: pc_precond_diag_blocks_only = .TRUE.
  ...
  LOGICAL, SAVE :: pc_tzinger_in_pc = .TRUE.
  ...
  ! 切线/PC 诊断：与 BuFlow_test_d.f90 报告 4.1–4.5 对齐；各 *_once 首拍后自关
...
&   dwdx, ncells, csvfilename)
  ...
  END SUBROUTINE PC_PERMUTE_BCSR_SYMMETRIC
	...
	SUBROUTINE BUILD_PC_AD_FULL_STENCIL(data_4d137, cellprimitives)
	  ...
	  DO c = 1_8, ncells
	    ...
	    DO mvar = 1_8, 5_8
	      ...
	      CALL SET_USE_JSTEPSS_FOR_RESIDUAL_D(.FALSE.)
	      CALL SET_PC_HICKEN_A1(.TRUE., pc_hicken_sigma)
	      CALL COMPUTE_STEADY_RESIDUAL_D(data_4d137, data_4d137d, cellprimitives, cellprimitivesd)
	      CALL SET_PC_HICKEN_A1(.FALSE.)
	      DO rcell = 1_8, ncells
	...
	END SUBROUTINE BUILD_PC_AD_FULL_STENCIL
	  ...
	  ! 着色同色互扰/填块正确性：采样颜色上做「单种子参考」并与同色批量种子填入 pc_blk 比对（BuFlow_test_d 报告 4.1）
	  IF (pc_check_color_interference_once) THEN
	    BLOCK
	      INTEGER(kind=8) :: nsamp_c, s_c, idx_c, cc_c, nsamp_colors_c
	      INTEGER(kind=8), ALLOCATABLE :: samp_cells_c(:), seen_color_c(:)
	      REAL(kind=8) :: diff2_c, ref2_c, rel_color_err_c
	      INTEGER(kind=8) :: iv_c, rv_c, pc_loc, jj_r, rcell_x
	      nsamp_c = MIN(3_8, ncolor)
	      ALLOCATE(samp_cells_c(nsamp_c), seen_color_c(nsamp_c))
	      samp_cells_c = 0_8
	      seen_color_c = 0_8
	      nsamp_colors_c = 0_8
	      DO cc_c = 1_8, ncells
	        IF (nsamp_colors_c >= nsamp_c) EXIT
	        IF (color(cc_c) < 1_8) CYCLE
	        DO s_c = 1_8, nsamp_colors_c
	          IF (seen_color_c(s_c) == color(cc_c)) EXIT
	        END DO
	        IF (s_c <= nsamp_colors_c) CYCLE
	        nsamp_colors_c = nsamp_colors_c + 1_8
	        seen_color_c(nsamp_colors_c) = color(cc_c)
	        samp_cells_c(nsamp_colors_c) = cc_c
	      END DO
	      diff2_c = 0.0_8
	      ref2_c = 0.0_8
	      data_4d137d = 0.0_8
	      DO idx_c = 1_8, nsamp_colors_c
	        cc_c = samp_cells_c(idx_c)
	        IF (cc_c < 1_8) CYCLE
	        DO iv_c = 1_8, 5_8
	          cellprimitivesd = 0.0_8
	          cellprimitivesd(cc_c, iv_c) = 1.0_8
	          CALL SET_USE_JSTEPSS_FOR_RESIDUAL_D(.FALSE.)
	          CALL SET_PC_HICKEN_A1(.TRUE., pc_hicken_sigma)
	          CALL COMPUTE_STEADY_RESIDUAL_D(data_4d137, data_4d137d, cellprimitives, cellprimitivesd)
	          CALL SET_USE_JSTEPSS_FOR_RESIDUAL_D(.FALSE.)
	          CALL SET_PC_HICKEN_A1(.FALSE.)
	          DO jj_r = 1_8, row_cnt(cc_c)
	            rcell_x = nlist(cc_c, jj_r)
	            IF (rcell_x < 1_8 .OR. rcell_x > ncells) CYCLE
	            pc_loc = FIND_BLOCK_POS(rcell_x, cc_c)
	            IF (pc_loc <= 0_8) CYCLE
	            DO rv_c = 1_8, 5_8
	              diff2_c = diff2_c + (pc_blk(pc_loc, rv_c, iv_c) - fluxresiduals_slnd(rcell_x, rv_c))**2
	              ref2_c = ref2_c + fluxresiduals_slnd(rcell_x, rv_c)**2
	            END DO
	          END DO
	        END DO
	      END DO
	      rel_color_err_c = SQRT(diff2_c) / MAX(1.0d-300, SQRT(ref2_c))
	      PRINT *, '[PC-COLOR-CHECK] sampled_colors=', nsamp_colors_c, ' rel_err=', rel_color_err_c
	      DEALLOCATE(samp_cells_c, seen_color_c)
	    END BLOCK
	  ...
	  ! z=ApplyPC(rhs) 后代回显式 A0（pc_A0）：||rhs-A0 z||/||rhs||（报告 4.5）；RCM 下跳过
	  IF (pc_ready .AND. pc_check_minv_accuracy_once) THEN
	    IF (pc_rcm_active) THEN
	      PRINT *, '[PC-MINV-CHECK] skip: RCM active (unknown reorder vs physical DOF)'
	      pc_check_minv_accuracy_once = .FALSE.
	    ELSE
	      BLOCK
	        INTEGER(kind=8) :: idx_m, worst_cell_m
	        REAL(kind=8) :: minv_rel, rmax_m, zmax_m, rloc_m
	        ...
	        DO idx_m = 1_8, 5_8*ncells
	          rhs_chk(idx_m) = SIN(1.7d-3*REAL(idx_m,kind=8)) + 0.2d0*COS(9.0d-4*REAL(idx_m,kind=8))
	        END DO
	        CALL APPLY_PC_INV(rhs_chk, z_chk)
	        CALL PC_APPLY_A0_EXPLICIT(ncells, z_chk, az_chk)
	        ...
	        worst_cell_m = -1_8
	        DO idx_m = 1_8, ncells
	          rloc_m = SQRT(SUM(r_chk((idx_m-1_8)*5_8+1_8:(idx_m-1_8)*5_8+5_8)**2))
	          IF (rloc_m > rmax_m) THEN
	            rmax_m = rloc_m
	            zmax_m = SQRT(SUM(z_chk((idx_m-1_8)*5_8+1_8:(idx_m-1_8)*5_8+5_8)**2))
	            worst_cell_m = idx_m
	          END IF
	        END DO
	        PRINT *, '[PC-MINV-CHECK] ||rhs-A0*ApplyPC(rhs)||/||rhs||=', minv_rel
	        PRINT *, '[PC-MINV-CHECK] worst_cell=', worst_cell_m, ' ||r_i||2=', rmax_m, ' ||z_i||2=', zmax_m
	...
	SUBROUTINE PC_APPLY_A0_EXPLICIT(ncells, x, y)
	  IMPLICIT NONE
	  INTEGER(kind=8), INTENT(IN) :: ncells
	  REAL(kind=8), INTENT(IN) :: x(:)
	  REAL(kind=8), INTENT(OUT) :: y(:)
	  INTEGER(kind=8) :: i, p, jc
	  REAL(kind=8) :: xj(5), yi(5), acc(5)
	  y = 0.0_8
	  IF (.NOT. ALLOCATED(pc_row_ptr) .OR. .NOT. ALLOCATED(pc_col_ind) .OR. .NOT. ALLOCATED(pc_A0)) RETURN
	  IF (SIZE(x) < 5_8*ncells .OR. SIZE(y) < 5_8*ncells) RETURN
	  DO i = 1_8, ncells
	    acc = 0.0_8
	    DO p = pc_row_ptr(i), pc_row_ptr(i+1_8) - 1_8
	      jc = pc_col_ind(p)
	      xj = x((jc-1_8)*5_8+1_8:(jc-1_8)*5_8+5_8)
	      CALL MATVEC5(pc_A0(p,:,:), xj, yi)
	      acc = acc + yi
	    END DO
	    y((i-1_8)*5_8+1_8:(i-1_8)*5_8+5_8) = acc
	  END DO
	END SUBROUTINE PC_APPLY_A0_EXPLICIT
	...
	SUBROUTINE BLOCK_INF_NORM5(A, nrm)
	  ...
	  INTEGER(kind=8) :: ir, jc
	  REAL(kind=8) :: rowsum
	  nrm = 0.0_8
	  DO ir = 1_8, 5_8
	    rowsum = 0.0_8
	    DO jc = 1_8, 5_8
	      rowsum = rowsum + ABS(A(ir,jc))
	    END DO
	    nrm = MAX(nrm, rowsum)
	  END DO
	...
	SUBROUTINE PCDBG_PRINT_MAT5(tag, A)
	  ...
	  DO rr = 1, 5
	    PRINT '(A,I1,A,5(1X,ES14.6))', ' row', rr, ':', A(rr,1), A(rr,2), A(rr,3), A(rr,4), A(rr,5)
	  END DO
	...
	SUBROUTINE FACTOR_BLOCK_ILU0(ncells, ok)
	  ...
	  INTEGER(kind=8) :: i_diag, worst_i, worst_p
	  REAL(kind=8) :: T(5,5), S(5,5)
	  REAL(kind=8) :: worst_cond, nd, ni, cnow, cmin, cmax, csum
	  LOGICAL :: inv_ok
	  ...
	  cmax = 0.0_8
	  csum = 0.0_8
	  DO i_diag = 1_8, ncells
	    p_ij = pc_diag_pos(i_diag)
	    IF (p_ij <= 0_8) CYCLE
	    CALL BLOCK_INF_NORM5(pc_blk(p_ij,:,:), nd)
	    CALL BLOCK_INF_NORM5(pc_uinv(i_diag,:,:), ni)
	    cnow = nd * ni
	    cmin = MIN(cmin, cnow)
	    cmax = MAX(cmax, cnow)
	    csum = csum + cnow
	    ...
	    END IF
	  END DO
	  PRINT *, '[PC-ILU] diag_cond_est min/max/avg=', cmin, cmax, csum / REAL(MAX(1_8, ncells), 8)
