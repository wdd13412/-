...
!    输出 P_tagent_x1..x4, T_tagent_x1..x4, U_tagent_x1..x4。用 F16.8 避免 ParaView "Unsupported cell attribute type"
      ! 4. cellprimitivesd = dwdx(ncells*5, 4), variable-major: dim1 = [all P][all T][all Ux][all Uy][all Uz], dim2 = param 1..4
            ! 4. cellprimitivesd = dwdx(ncells*5, nparams), param 1..4 = x1..x4, 5 = mach
      ...
      DO j = 1, nparams
        ...
        WRITE(fileunit, '(A)') 'LOOKUP_TABLE default'
        DO i = 1, ncells
          t_t = cellprimitivesd(ncells + i, j)
        ...
        END IF
        DO i = 1, ncells
          ux_t = cellprimitivesd(2*ncells + i, j)
          uy_t = cellprimitivesd(3*ncells + i, j)
          uz_t = cellprimitivesd(4*ncells + i, j)
      ...
      IF (PRESENT(rhs_P)) THEN
        DO j = 1, SIZE(rhs_P, 2)
          IF (j .LE. 4) THEN
            WRITE(fileunit, '(A,I0,A)') 'SCALARS RHS_P_x', j, ' double 1'
          ELSE
            WRITE(fileunit, '(A)') 'SCALARS RHS_P_mach double 1'
          END IF
          WRITE(fileunit, '(A)') 'LOOKUP_TABLE default'
          DO i = 1, ncells
...
&     2), uunitvec(3)/)
    ...
    boundaryconditions(4)%params(2:5) = 0.0_8
    meshpath = 'mesh/OFairfoilMesh'
    CALL READOPENFOAMMESH_D(meshpath, point_update, point_updated, &
&                     tempmesh)
...
&               - cellprimitives(owner_cell, 1)*favecs_mesh(face_idx, 1)
  ...
  subroutine solve_adjoint_jfgmres(b, psi, n, maxit, tol, matvec_at, niter, resnorm)
		implicit none
		integer(kind=8), intent(in) :: n, maxit
		real(kind=8), intent(in) :: b(n), tol
		real(kind=8), intent(inout) :: psi(n)
		procedure(adjoint_matvec_interface), optional :: matvec_at
		integer(kind=8), intent(out), optional :: niter
		real(kind=8), intent(out), optional :: resnorm
		real(kind=8), allocatable :: r(:), v(:,:), h(:,:), cs(:), sn(:), y(:), e1(:), tmp(:)
		real(kind=8) :: bnrm, rnrm, tol_b
		integer(kind=8) :: k, j, m, i
		integer(kind=8), parameter :: m_restart = 50
		if (maxit <= 0) return
		allocate(r(n), v(n, m_restart+1), h(m_restart+1, m_restart), cs(m_restart), sn(m_restart), y(m_restart), e1(m_restart+1), tmp(n))
		psi = 0.0_8
		r = b
		if (present(matvec_at)) then
			call matvec_at(n, psi, tmp)
			r = r - tmp
		end if
		bnrm = sqrt(sum(b*b))
		if (bnrm < 1.0e-30_8) bnrm = 1.0_8
		tol_b = tol * bnrm
		rnrm = sqrt(sum(r*r))
		if (present(resnorm)) resnorm = rnrm
		m = 0
		do k = 1, maxit
			v(:, 1) = r / rnrm
			e1 = 0.0_8
			...
			do j = 1, m_restart
				m = j
				if (.not. present(matvec_at)) exit
				call matvec_at(n, v(1:n, j), tmp)
				v(1:n, j+1) = tmp
				do i = 1, j
					h(i, j) = dot_product(v(1:n, i), v(1:n, j+1))
					v(1:n, j+1) = v(1:n, j+1) - h(i, j) * v(1:n, i)
				end do
				h(j+1, j) = sqrt(sum(v(1:n, j+1)**2))
				if (h(j+1, j) < 1.0e-30_8) exit
				v(1:n, j+1) = v(1:n, j+1) / h(j+1, j)
				do i = 1, j-1
					call givens_apply(cs(i), sn(i), h(1:j+1, j))
				end do
				call givens(h(j, j), h(j+1, j), cs(j), sn(j))
				call givens_apply(cs(j), sn(j), h(1:j+1, j))
				e1(j+1) = -sn(j) * e1(j)
				e1(j) = cs(j) * e1(j)
				rnrm = abs(e1(j+1))
				if (present(resnorm)) resnorm = rnrm
			...
			y(1:m) = e1(1:m)
			do j = m, 1, -1
				y(j) = (y(j) - dot_product(h(j, j+1:m), y(j+1:m))) / h(j, j)
			end do
			psi = psi + matmul(v(1:n, 1:m), y(1:m))
			if (rnrm <= tol_b) exit
			r = matmul(v(1:n, 1:m+1), e1(1:m+1))
			rnrm = sqrt(sum(r*r))
		end do
		if (present(niter)) niter = k
		deallocate(r, v, h, cs, sn, y, e1, tmp)
	contains
		...
		subroutine givens_apply(c, s, vec)
			real(kind=8), intent(in) :: c, s
			real(kind=8), intent(inout) :: vec(:)
			real(kind=8) :: t
			t = c * vec(size(vec)-1) - s * vec(size(vec))
			vec(size(vec)) = s * vec(size(vec)-1) + c * vec(size(vec))
			vec(size(vec)-1) = t
		end subroutine
	end subroutine solve_adjoint_jfgmres
  SUBROUTINE TANGENT_MATVEC(data_4d137, cellprimitives, n, v, av)
    ...
    cellprimitivesd = RESHAPE(v, (/ncells, 5_int64/))
    data_4d137d = 0.0_8
    CALL COMPUTE_STEADY_RESIDUAL_D(data_4d137, data_4d137d, cellprimitives, cellprimitivesd)
    ! 結果寫在模塊變量 fluxresiduals_slnd
    av = RESHAPE(fluxresiduals_slnd(1:ncells, 1:5), (/n/) )
  ...
  SUBROUTINE TANGENT_RHS_I(data_4d137, cellprimitives, n, i_param, b_i)
    ...
    CALL COMPUTE_STEADY_RESIDUAL_D(data_4d137, data_4d137d, cellprimitives, cellprimitivesd)
    b_i = -RESHAPE(fluxresiduals_slnd(1:ncells, 1:5), (/n/) )
  ...
  END SUBROUTINE TANGENT_RHS_MACH
	...
	SUBROUTINE SOLVE_TANGENT_JFGMRES(data_4d137, cellprimitives, n, b, x, tol, maxiter, m_restart, info)
	  ...
	  REAL(kind=8), INTENT(INOUT) :: x(n)!dw/dx
	  INTEGER(kind=8), INTENT(IN) :: n, maxiter, m_restart
	  INTEGER, INTENT(OUT) :: info

	  INTEGER(kind=8), PARAMETER :: mm = 50
	  REAL(kind=8), PARAMETER :: eps_small = 1.0e-30_8

	  REAL(kind=8), ALLOCATABLE :: r(:), v(:, :), w(:), h(:, :), cs(:), sn(:), y(:), e1(:)
	  REAL(kind=8) :: bnrm, rnrm, tol_b
	  INTEGER(kind=8) :: k, j, m, i
	  ...
	  ! 工作数组：r 残差，v 正交基，w 存 A*v（必须分配），h Hessenberg，cs/sn Givens，y 最小二乘解，e1 右端
	  ALLOCATE(r(n), v(n, mm+1), w(n), h(mm+1, mm), cs(mm), sn(mm), y(mm), e1(mm+1))

	  !----------------------------------------------------------------------------
	  ! 初始残差 r = b - A*x，收敛阈值 tol_b = tol*||b||
	  !----------------------------------------------------------------------------
	  bnrm = SQRT(SUM(b*b))
	  ...
	  r = r - w
	  rnrm = SQRT(SUM(r*r))
	  PRINT *, '  [GMRES] ||b||=', bnrm, ' tol*||b||=', tol_b, ' ||r0||=', rnrm, ' ||r0||/||b||=', rnrm/bnrm

	  IF (rnrm <= tol_b) THEN
		info = 0
		DEALLOCATE(r, v, w, h, cs, sn, y, e1)
		RETURN
	  END IF
	  m = 0

	  !======================================================================
	  ! 外循环：GMRES 重启，最多 maxiter 次
	  !======================================================================
	  DO k = 1, maxiter
		v(1:n, 1) = r / rnrm
		e1 = 0.0_8
		...
		!----------------------------------------------------------------------------
		! 内循环：Arnoldi 过程，构造 Krylov 基与 Hessenberg 矩阵 H
		!----------------------------------------------------------------------------
		DO j = 1, mm
		  m = j
		  CALL TANGENT_MATVEC(data_4d137, cellprimitives, n, v(1:n, j), w)
		  v(1:n, j+1) = w
		  ...
		  END DO
		  h(j+1, j) = SQRT(SUM(v(1:n, j+1)**2))
		  IF (h(j+1, j) < eps_small) EXIT
		  v(1:n, j+1) = v(1:n, j+1) / h(j+1, j)

		  DO i = 1, j - 1
		    CALL givens_apply_t(cs(i), sn(i), h(1:j+1, j))
		  END DO
		  CALL givens_t(h(j, j), h(j+1, j), cs(j), sn(j))
		  CALL givens_apply_t(cs(j), sn(j), h(1:j+1, j))
		  e1(j+1) = -sn(j) * e1(j)
		  e1(j) = cs(j) * e1(j)
		  rnrm = ABS(e1(j+1))
		  PRINT *, '(restart)k=', k, ' j=', j, ' ||r||/||b||=', rnrm/bnrm
		  IF (rnrm <= tol_b) EXIT
		END DO
		...
		!----------------------------------------------------------------------------
		! 回代求解上三角系统 H(1:m,1:m)*y(1:m) = e1(1:m)
		!----------------------------------------------------------------------------
		y(1:m) = e1(1:m)
		DO j = m, 1, -1
		  IF (j < m) THEN
		  ...
		  END IF
		END DO
		x = x + MATMUL(v(1:n, 1:m), y(1:m))
		PRINT *, '  [GMRES] outer=', k, ' ||r||=', rnrm, ' ||r||/||b||=', rnrm/bnrm, ' converged=', (rnrm <= tol_b)
		...
		!----------------------------------------------------------------------------
		! 下一轮残差 r = V(:,1:m+1)*e1(1:m+1)（与伴隨一致）
		!----------------------------------------------------------------------------
		r(1:n) = MATMUL(v(1:n, 1:m+1), e1(1:m+1))
		rnrm = SQRT(SUM(r*r))
	  END DO

	  IF (info == 2 .AND. rnrm <= tol_b) info = 0
	  IF (info == 2 .AND. rnrm > tol_b) info = 1
	  DEALLOCATE(r, v, w, h, cs, sn, y, e1)

	CONTAINS
	  SUBROUTINE givens_t(a, b, c, s)
		REAL(kind=8), INTENT(IN) :: a, b
		REAL(kind=8), INTENT(OUT) :: c, s
		REAL(kind=8) :: r
		r = SQRT(a*a + b*b)
		IF (ABS(r) < eps_small) THEN
		  c = 1.0_8
		  s = 0.0_8
		ELSE
		  c = a / r
		  s = -b / r
		END IF
	  END SUBROUTINE givens_t
	  SUBROUTINE givens_apply_t(c, s, vec)
		REAL(kind=8), INTENT(IN) :: c, s
		REAL(kind=8), INTENT(INOUT) :: vec(:)
		REAL(kind=8) :: t
		INTEGER(kind=8) :: sz
		sz = SIZE(vec)
		IF (sz < 2) RETURN
		t = c * vec(sz-1) - s * vec(sz)
		vec(sz) = s * vec(sz-1) + c * vec(sz)
		vec(sz-1) = t
	  END SUBROUTINE givens_apply_t
	END SUBROUTINE SOLVE_TANGENT_JFGMRES
	! 輸出翼型壁面壓力係數 Cp 和 dCp/dx 到 CSV（前99個點=翼型壁面，數據用單元平均移到點上）
