...
CONTAINS
!  Differentiation of airfoil_deformation_hh in forward (tangent) mode (with options i4 dr8 r4):
...
! 处理变形参数
    ...
    DO i=1,d
      DO k=1,SIZE(data_4d137, 1)
        arg1d = -(3.1415926535_8*data_4d137d(k, i)/2.0_8)
        arg1 = (1.0_8-data_4d137(k, i))/2.0_8*3.1415926535_8
        sd(k, i) = -(0.05_8*SIN(arg1)*arg1d)!0.05_8
        s(k, i) = 0.05_8*COS(arg1)!0.05_8
      END DO
    END DO
    s_upd = sd(:, 1:2)
    s_up = s(:, 1:2)
...
! 计算变形
    IF (n .GT. 0) THEN
! 计算变形函数
! 第二个参数 50 是 x1 的长度,这个函数FUN1主控前缘第一个点
      CALL FUN1(x1, 50, fun1_result)
! 第二个参数 50 是 X（即 x1）的长度,这个函数FUNK选一个点（第几个横坐标点）主控
      CALL FUNK(x1, 50, 2, funk_result)
      !因此第一个参数主控上表面前缘，第三个参数主控下表面前缘，第二个选一个上表面主控点，第四个选一个下表面点主控
      ...
      yy_low = y_low + f_low
! 特殊处理重叠点
      avg_y_51d = (yy_upd(50)+yy_lowd(1))/2.0_8
...
!		if (allocated(wing)) deallocate(wing)				
      CALL MESHDEFORMATION_D(wing_update, wing_updated)
...
! 用 n 声明 result 的维度
      ...
      INTRINSIC LOG10
      INTRINSIC SIN
      REAL*8, DIMENSION(n) :: arg10
      xxx = (/x1(1), x1(8)/)
      ee = LOG10(0.5_8)/LOG10(xxx(k))
      arg10 = 3.1415926535_8*x**ee
      result = SIN(arg10)**3
    END SUBROUTINE FUNK
...
!   Plus diff mem management of: point_update:in wing_update:in
  SUBROUTINE MESHDEFORMATION_D(wing_update, wing_updated)
    IMPLICIT NONE
...
&   weights_y(:)
    REAL(kind=8), ALLOCATABLE :: wing_update_coordsd(:, :), d_wing_xd(:)&
&   , d_wing_yd(:), weights_xd(:), weights_yd(:)
...
!		allocate(wing_coords(nWing,2), wing_update_coords(nWing,2), inoutput_coords(nInoutput,2))
    ...
    wing_coords = wing
    IF (ALLOCATED(wing_update_coordsd)) wing_update_coordsd = 0.0_8
    wing_update_coordsd = wing_updated
...
&                       ncontrol)
      phi(j, :) = phi_row
    END DO
    ALLOCATE(weights_xd(ncontrol), source=0.0_8)
    ALLOCATE(weights_x(ncontrol), source=0.0_8)
! 预初始化
    ALLOCATE(weights_yd(ncontrol), source=0.0_8)
...
!计算网格点的位移d_mesh_x,d_mesh_y
      CALL DOT_PRODUCT_REALLL_D(weights_x, weights_xd, phi_row, d_mesh_x&
&                         (pt), d_mesh_xd(pt))
      CALL DOT_PRODUCT_REALLL_D(weights_y, weights_yd, phi_row, d_mesh_y&
&                         (pt), d_mesh_yd(pt))
    END DO
    point_updated = 0.0_8
...
!  Differentiation of matmul_real in forward (tangent) mode (with options i4 dr8 r4):
...
!  Differentiation of dot_product_realll in forward (tangent) mode (with options i4 dr8 r4):
!   variations   of useful results: result
!   with respect to varying inputs: a
! 向量点积子程序：result = A · B（A和B需等长）
  SUBROUTINE DOT_PRODUCT_REALLL_D(a, ad, b, result, resultd)
    IMPLICIT NONE
...
! 初始化结果
      ...
      END DO
    END IF
  END SUBROUTINE DOT_PRODUCT_REALLL_D
...
! 初始化结果
  ...
  SUBROUTINE GAUSSIAN_RBF_ARRAY(r_array, epsilon, phi_array, n)
    ...
    REAL(kind=8), PARAMETER :: d_nomal=1.0_8/4.5_8!这里最小为4.5，因为4.5为控制点到网格点的最大距离,原始值取的是4.5_8  54_8
    INTEGER :: i
    DO i=1,n
      phi_array(i) = (1.0_8-d_nomal*r_array(i))**2
      !phi_array(i) = 1.0_8-d_nomal*r_array(i)
    END DO
  END SUBROUTINE GAUSSIAN_RBF_ARRAY

  ! RBF_TYPE: 1=(1-r/scale)^2 紧支撑; 2=逆多二次 1/sqrt(1+(r/scale)^2) 全域
  SUBROUTINE GAUSSIAN_RBF_ARRAYY(r_array, epsilon, phi_array, n)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: n
    REAL(kind=8), INTENT(IN) :: r_array(n)
    REAL(kind=8), INTENT(IN) :: epsilon
    REAL(kind=8), INTENT(OUT) :: phi_array(n)
    REAL(kind=8), PARAMETER :: RBF_SCALE = 4.5_8!25.0_8
    ...
    DO i = 1, n
      ri = r_array(i)
      rsc = ri / RBF_SCALE
      IF (RBF_TYPE .EQ. 1) THEN
        phi_array(i) = MAX(0.0_8, 1.0_8 - rsc)**2
      ELSE
        phi_array(i) = 1.0_8 / SQRT(1.0_8 + rsc)!很奇怪的是:原来的值rsc*rsc报错(生成vtk用paraview打开时出现错误0个单元),似乎由于控制点距离由接近0.002到4.5这个范围，对rsc加平方后，会导致不同点的变形幅度差异太大
      END IF
    END DO
  END SUBROUTINE GAUSSIAN_RBF_ARRAYY
