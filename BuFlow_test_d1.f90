...
! 日後可用解析 JST-L1 係數寫入 pc_blk_inv，介面不變。
  ...
  ! 右预 GMRES 校正：数学上须 x += M^{-1}(V·y)。仅当 APPLY_PC_INV 对向量线性时才有
  ! M^{-1}(V·y) = Σ y_j M^{-1} v_j（BuFlow_test_d 主路径 ILU+equil 近似线性故可用 z_basis*y）。
  ! (47) 上 AM1/多层等使 M^{-1} 非线性：z_basis*y 与真更新偏离，m 越大 y 越稠 → 真残差爆炸。
  LOGICAL, SAVE :: gmres_right_pc_linear_combo_update = .FALSE.
  ! 与 BuFlow_test_d.f90 对齐：Arnoldi 两遍 MGS；大三遍当 m_restart>90
  LOGICAL, SAVE :: gmres_arnoldi_two_pass = .TRUE.
  LOGICAL, SAVE :: pc_tangent_gmres_krylov_diag_once = .TRUE.
...
&               - cellprimitives(owner_cell, 1)*favecs_mesh(face_idx, 1)
  ...
  ! 与 BuFlow_test_d.f90 的 PC_RUN_TANGENT_RHS_ALIGNED_DIAGNOSTICS 对齐：探针为 GMRES 首方向 v(:,1)（及右预下 Z1）。
  SUBROUTINE PC_RUN_TANGENT_GMRES_KRYLOV_DIAG47(data_4d137, cellprimitives, n, v1_unit)
    IMPLICIT NONE
    REAL(kind=8), INTENT(IN) :: data_4d137(1, 4)
    REAL(kind=8), INTENT(IN) :: cellprimitives(:, :)
    INTEGER(kind=8), INTENT(IN) :: n
    REAL(kind=8), INTENT(IN) :: v1_unit(n)
    INTEGER(kind=8) :: ncells, c, ib, bidx, ipass, npass
    REAL(kind=8), ALLOCATABLE :: vwork(:), jv_tan(:), jv_a0(:), z_ilu(:), az(:), z_full(:)
    REAL(kind=8) :: nv_probe, ntan, na0, ndiff, nr, worst_val, nfull_res, rloc
    INTEGER(kind=8) :: worst_cell
    CHARACTER(len=14) :: ctag
    LOGICAL :: do_j, do_m, do_z_pass

    ncells = n / 5_8
    IF (ncells <= 0_8 .OR. ncells * 5_8 /= n) THEN
      PRINT *, '[PC-DIAG-GMRES47] skip: n not multiple of 5, n=', n
      RETURN
    END IF
    IF (ncells /= pc_ncells) THEN
      PRINT *, '[PC-DIAG-GMRES47] skip: ncells=', ncells, ' pc_ncells=', pc_ncells
      RETURN
    END IF
    nv_probe = SQRT(SUM(v1_unit(1:n) * v1_unit(1:n)))
    IF (nv_probe <= 1.0d-300) THEN
      PRINT *, '[PC-DIAG-GMRES47] skip: ||v1|| tiny'
      RETURN
    END IF
    do_j = pc_a0_ready .AND. ALLOCATED(pc_a0_blk)
    do_m = do_j
    do_z_pass = pc_ready
    npass = 1
    IF (do_z_pass) npass = 2
    PRINT *, '[PC-DIAG-GMRES47] probe from GMRES v(:,1); pc_ready=', pc_ready, ' jmatvec=', do_j, ' minv=', do_m
    IF (do_z_pass) PRINT *, '[PC-DIAG-GMRES47] also pass Z1=M^{-1}v1 (unnorm) for Jv vs A0'

    DO ipass = 1, npass
      ALLOCATE(vwork(n))
      IF (ipass == 1) THEN
        vwork(1:n) = v1_unit(1:n)
        ctag = 'GMRES-V1    '
      ELSE
        CALL APPLY_PC_INV(v1_unit(1:n), vwork(1:n))
        ctag = 'GMRES-Z1    '
      END IF
      ...
      IF (nv_probe <= 1.0d-300) THEN
        ...
        CYCLE
      END IF
      PRINT *, '[PC-DIAG-GMRES47] pass', ipass, 'tag=', ctag, '||probe||=', nv_probe

      IF (do_j) THEN
        ALLOCATE(jv_tan(n), jv_a0(n))
        ...
        DEALLOCATE(jv_tan, jv_a0)
      ELSE
        PRINT *, '[PC-DIAG-GMRES47]', ctag, 'skip Jv/A0: pc_a0_ready=', pc_a0_ready, ' alloc(a0_blk)=', ALLOCATED(pc_a0_blk)
      END IF

      IF (do_m .AND. ipass == 1) THEN
        ALLOCATE(z_ilu(n), az(n), z_full(n))
        CALL PC_APPLY_ILU_CORE(vwork, z_ilu)
        CALL PC_APPLY_A0(ncells, z_ilu, az)
        nr = SQRT(SUM((vwork(1:n) - az(1:n)) * (vwork(1:n) - az(1:n))))
        nfull_res = SQRT(SUM(az(1:n) * az(1:n)))
        PRINT *, '[PC-P-LU-DIR]', ctag, 'z=ILU_core(rhs) ||A0*z||=', nfull_res, ' ||rhs||=', nv_probe
        PRINT *, '[PC-MINV-CHECK-CORE]', ctag, '||rhs - A0*z_ilu||/||rhs||=', nr / MAX(1.0d-300, nv_probe)
        CALL APPLY_PC_INV(vwork, z_full)
        CALL PC_APPLY_A0(ncells, z_full, az)
        nr = SQRT(SUM((vwork(1:n) - az(1:n)) * (vwork(1:n) - az(1:n))))
        PRINT *, '[PC-P-LU-DIR]', ctag, 'z=APPLY_PC_INV(rhs) ||A0*z||=', SQRT(SUM(az(1:n) * az(1:n)))
        PRINT *, '[PC-MINV-CHECK]', ctag, '||rhs - A0*z||/||rhs||=', nr / MAX(1.0d-300, nv_probe), &
          & ' ||z||=', SQRT(SUM(z_full(1:n) * z_full(1:n)))
        DEALLOCATE(z_ilu, az, z_full)
      END IF
      DEALLOCATE(vwork)
    END DO
  END SUBROUTINE PC_RUN_TANGENT_GMRES_KRYLOV_DIAG47
...
!	  PRINT *, '[REG] eps_reg=', eps_reg, ' eps_reg/||b||=', eps_reg/bnrm
	...
	&    ' ||r0_true||=', rnrm_true, ' ||r0_true||/||b||=', rnrm_true/bnrm
	  ...
	  IF (rnrm_true <= tol_b) THEN
		info = 0
		DEALLOCATE(r, v, z_basis, w, h, y, z, tmp)
		RETURN
	  END IF
...
&        ' res_ls/(eta_k*beta)=', res_ls / MAX(1.0d-300, eta_k*beta_kry)
		...
		PRINT *, '[GMRES-DBG] outer=', k, ' beta_kry=', beta_kry, &
				 ' ||r||=', rcheck, ' ||v1||=', v1norm
		IF (k == 1_8 .AND. pc_tangent_gmres_krylov_diag_once .AND. pc_a0_ready .AND. ALLOCATED(pc_a0_blk)) THEN
		  CALL PC_RUN_TANGENT_GMRES_KRYLOV_DIAG47(data_4d137, cellprimitives, n, v(1:n, 1))
		  pc_tangent_gmres_krylov_diag_once = .FALSE.
		END IF
...
!		  w(1:n) = w(1:n) + eps_reg * z(1:n)   ! (A + eps I) z
		  ...
		  ! 修正 Gram–Schmidt；第二遍与 BuFlow_test_d.f90 的 gmres_arnoldi_two_pass 一致
		  DO i = 1, j
		  ...
		  IF (gmres_arnoldi_two_pass) THEN
		    ...
		    END DO
		  END IF
		  ! 大 m_restart 再正交一遍，减轻 MGS 漂移（非线性 PC 下仍须配合正确更新公式）
		  IF (gmres_arnoldi_two_pass .AND. m_restart > 90_8) THEN
		    DO i = 1, j
		      tmgs = DOT_PRODUCT(v(1:n, i), v(1:n, j+1))
		      h(i, j) = h(i, j) + tmgs
		      v(1:n, j+1) = v(1:n, j+1) - tmgs * v(1:n, i)
		...
		! ===================== 解更新：右预 =====================
		! 非线性 PC：须 x += M^{-1}(V·y)；线性 PC 下可与 Σ y_j M^{-1}v_j 互换（gmres_right_pc_linear_combo_update）
		...
		PRINT *, '[GMRES-DBG] outer=', k, ' ||Vy||=', tmpnrm, ' ||x||=', xnrm, ' ||Vy||/||x||=', drel
		IF (gmres_right_pc_linear_combo_update .AND. pc_ready) THEN
		  z(1:n) = MATMUL(z_basis(1:n, 1:m_dim), y(1:m_dim))
		  IF (k == 1_8 .AND. m_dim == m_restart) PRINT *, '[GMRES-DBG] update path: Z_basis*y (linear-combo; gmres_right_pc_linear_combo_update=T)'
		ELSE IF (pc_ready) THEN
		  CALL APPLY_PC_INV(tmp(1:n), z(1:n))
		  IF (k == 1_8 .AND. m_dim == m_restart) PRINT *, '[GMRES-DBG] update path: M^{-1}(V*y) single APPLY_PC_INV (nonlinear-safe)'
		ELSE
		  z(1:n) = tmp(1:n)
		END IF
